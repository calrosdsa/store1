 ...
 export default function Login({providers}){
  ...
  return(
      ...
      {Object.values(providers).map(provider => (
          ...
          <button className="my-1 -ml-3 text-xs md:text-sm font-bold p-2 bg-blue-500 text-gray-200 min-w-[160px] md:min-w-[200px] h-8 md:h-9" onClick={() => signIn(provider.id)}>Sign in with {provider.name}</button>
        </div>
      ))}
      </div>
          </div>
